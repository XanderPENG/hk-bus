...
def clearing(df):
    df.columns = ['solution_no', 'obj1', 'obj2', 'obj3']
    ...
    df['obj3'] = df_numeric['obj3']

    # # 去重
    rows_to_remove = []
    for index, row in df.iterrows():
        # 检查是否存在其他行的所有列值都大于当前行
        if any((df[['obj1','obj2','obj3']] < row.iloc[-3:]).all(axis=1)):
    ...
    df_numeric_nodpl['obj3'] = df_numeric_nodpl['obj3']
