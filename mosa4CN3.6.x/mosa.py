...
class MOSA(MOSAConfig):
    ...
    def output_fitness(self):
        fitness_df = pd.DataFrame(self.fitness, columns=['iter', 'Obj1', 'Obj2'])
        objectives = ['Obj1', 'Obj2']

        # 对每个目标进行归一化
        for obj in objectives:
            initial = fitness_df[obj].iloc[0]  # 起点值
            final = fitness_df[obj].iloc[-1]  # 终点值
            if initial != final:  # 避免除以零的情况
                fitness_df[f'{obj}_norm'] = (fitness_df[obj] - final) / (initial - final)
            else:
                fitness_df[f'{obj}_norm'] = 0  # 如果起点和终点相同，设为常值（这里用 0）

        # 绘制图形
        plt.figure(figsize=(10, 6))  # 设置图形大小
        for obj in objectives:
            plt.plot(fitness_df['iter'], fitness_df[f'{obj}_norm'], label=obj)
            ...
            for i in range(self.annealing_iters):  # The inner loop for looking for new solutions under a certain temp
                ...
                if is_early_terminate:  # When the early termination is set, judge whether it should stop
                    if no_eliminated_times > max_no_eliminated:
                        ...
                        break
                if self.is_tested:
                    total_obj1 = sum(item[1][0] for item in self.archive.values())
                    total_obj2 = sum(item[1][1] for item in self.archive.values())
                    n = len(self.archive)
                    self.fitness.append([all_iter_counts, -total_obj1/n,  -total_obj2/n])
